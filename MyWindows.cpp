...
MyResult MyWindows::WindowControls() { // ---- Window Controls
									   // ------------------------------------------------
	...
	// Begin/End ficam DENTRO das chaves — End() é sempre pareado com Begin().
	{
		...
		if (m_App->g_Settings->window.show_window_controls) {
			...
			if (ImGui::SliderFloat("Window Opacity", &m_App->g_window_opacity, 0.1f, 1.0f))
				SDL_SetWindowOpacity(m_App->g_Window,
									 m_App->g_window_opacity); // aplica imediatamente
			if (m_App->g_window_opacity < 1.0f) {
			...
			if (ImGui::SliderFloat("Font Scale", &m_App->g_Settings->font.font_scale_main, 0.5f,
								   3.0f, "%.2f")) {
			...
			ImGui::Text("Application average %.3f ms/frame (%.1f FPS)",
						1000.0f / m_App->g_io->Framerate, m_App->g_io->Framerate);
			...
			if (m_App->g_Logo.IsLoaded()) {
				...
				if (ImGui::IsItemHovered())
					ImGui::SetTooltip("Logo %dx%d", m_App->g_Logo.GetWidth(),
									  m_App->g_Logo.GetHeight());
			}
			...
			ImGui::Separator();
			if (ImGui::CollapsingHeader("ImPlot3D")) {
				if (ImGui::Checkbox("ImPlot3D RealtimePlots", &m_App->bImPlot3d_DemoRealtimePlots))
					m_App->SaveConfig();
				if (ImGui::Checkbox("ImPlot3D QuadPlots", &m_App->bImPlot3d_DemoQuadPlots))
					m_App->SaveConfig();
				if (ImGui::Checkbox("ImPlot3D TickLabels", &m_App->bImPlot3d_DemoTickLabels))
					m_App->SaveConfig();
			}
			ImGui::Separator();
...
MyResult MyWindows::Console() { // ---- Console ImGui interno
								// ------------------------------------------
...
MyResult MyWindows::StyleEditor() { // ---- Style Editor
									// ---------------------------------------------------
...
MyResult MyWindows::Graphs() { // ---- Gráfico de exemplo (ImPlot)
							   // ------------------------------------
	if (m_App->g_grafico) {
...
MyResult MyWindows::Graphs3D() { // ---- ImPlot3D demos
								 // -------------------------------------------------
