...
void Console::LoadEmojiFont(float font_size) {
	...
	merge_cfg.PixelSnapH = true;    // Align emoji to pixel boundaries; prevents blurriness
...
/**
 * @brief Divide uma linha de comando larga em tokens separados por espaços.
 *
 * O primeiro token é o nome do comando; os seguintes são os argumentos.
 * Múltiplos espaços consecutivos são ignorados — tokens nunca ficam vazios.
 *
 * Exemplos:
 ...
 *   L"exit"            → { L"exit" }
 *
 * @param line  Linha larga a tokenizar (não modificada).
 * @return      std::vector<std::wstring> com os tokens em ordem.
 */
static std::vector<std::wstring> TokenizeLine(const std::wstring& line)
{
    std::vector<std::wstring> tokens; // vetor resultado — tokens em ordem de aparição
    std::wstring              token;  // acumulador do token sendo construído agora

    for (const wchar_t ch : line)         // itera cada caractere largo da linha
    {
        if (ch == L' ' || ch == L'\t')    // separador: espaço ou tabulação larga
        {
            if (!token.empty())           // só salva se há conteúdo no acumulador
            {
                tokens.push_back(std::move(token)); // move o token para o vetor
                token.clear();                      // reinicia o acumulador para o próximo token
            }
            // separadores consecutivos são ignorados — não geram tokens vazios
        }
        else
        {
            token += ch; // acumula o caractere no token atual
        }
    }

    if (!token.empty())                   // token final sem separador após ele
        tokens.push_back(std::move(token)); // salva o último token

    return tokens; // NRVO — sem cópia extra no retorno
}
 ...
 *
 * Internamente envolve o handler num wrapper void() que:
 *  1. Lê InputBuf (wchar_t[]) — o texto bruto que o usuário digitou.
 *  2. Tokeniza a linha inteira com TokenizeLine().
 *  3. Remove o token[0] (nome do comando) e repassa o restante como
 *     std::vector<std::wstring> ao handler real.
 *
 * Isso mantém o DispatchTable com tipo uniforme
 * std::map<std::wstring, std::function<void()>> sem quebrar nada.
 *
 * @param name  Nome largo do comando (qualquer capitalização).
 * @param func  Handler void(std::vector<std::wstring>).
 */
    ...
    std::function<void(std::vector<std::wstring>)>    func)
{
    ...
    // O lambda resultante não recebe parâmetros — compatível com
    // std::function<void()> do DispatchTable existente.
    RegisterCommand(name, [this, func]()
    {
        // 1. Copia o conteúdo do buffer de entrada largo para uma wstring.
        //    InputBuf é wchar_t[512] — membro privado de Console.
        const std::wstring raw_line(InputBuf); // cópia segura; InputBuf não é modificado

        // 2. Divide a linha em tokens largos individuais.
        //    ex.: L"theme dark" → { L"theme", L"dark" }
        std::vector<std::wstring> tokens = TokenizeLine(raw_line);

        // 3. Descarta token[0] (nome do comando) e constrói o vetor de argumentos.
        //    Se o usuário digitou apenas o nome sem argumentos, args fica vazio.
        std::vector<std::wstring> args; // vetor que será entregue ao handler

        if (tokens.size() > 1) // há pelo menos um argumento além do nome
        {
            // Copia os tokens a partir do índice 1 (primeiro argumento real)
            // até o final do vetor de tokens.
            args.assign(
                tokens.begin() + 1, // iterador para o primeiro argumento
                tokens.end());      // iterador um-além do último token
        }
        // se tokens.size() <= 1, args permanece vazio — handler trata o caso

        func(std::move(args)); // entrega o vetor ao handler; move evita cópia
    });
}
 ...
 *
 * Delega ao RegisterCommand(name, func) acima e registra a descrição
 * separadamente em HelpDescriptions — mesmo padrão das outras sobrecargas.
 *
 * @param name  Nome largo do comando.
 * @param desc  Descrição larga exibida pelo comando HELP.
 * @param func  Handler void(std::vector<std::wstring>).
 */
void Console::RegisterCommand(
    const std::wstring&                               name,
    ...
    std::function<void(std::vector<std::wstring>)>    func)
{
    // Registra a lógica do comando (autocomplete + dispatch com args)
    RegisterCommand(name, func); // delega para a sobrecarga acima

    // Calcula a chave uppercase para HelpDescriptions — mesmo padrão
    // das outras sobrecargas existentes no sistema.
    std::wstring key = name;
    std::transform(
        key.begin(), key.end(),  // intervalo de entrada
        key.begin(),             // intervalo de saída (in-place)
        [](const wchar_t c) {
            return static_cast<wchar_t>(towupper(c)); // converte para maiúscula larga
        });

    HelpDescriptions[key] = desc; // armazena a descrição para o comando HELP
}
// ============================================================================
